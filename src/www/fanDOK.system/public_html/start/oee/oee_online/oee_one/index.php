...
$id_obor_get = urldecode ($_GET['id']);

$sql = "SELECT oborudovanie.id AS id, oborudovanie.name AS name, oborudovanie.code AS code, oborudovanie.group_id AS group_id, oborudovanie.tablename AS tablename, (SELECT name FROM group_o WHERE id=oborudovanie.code) AS groupname FROM oborudovanie  WHERE oee=1 AND name = '".$id_obor_get."' ORDER BY groupname";
$result = $conn->query($sql);
    $r=0;
    while($row = $result->fetch_assoc()) {
