...
$sql = "SELECT * FROM agregats WHERE parentid=0";
$result = $conn->query($sql);
    $r=0;
    while($row = $result->fetch_assoc()) {
        $oborudovanie[$r]=$row;
...
$sql = "INSERT INTO agregats (`name`, `parentid`, `oborid`) VALUES (' ".$_GET['agrparent']."',".$_GET['oborudovanie'].",".$oborid.");";
$result = $conn->query($sql);
header("Location: /start/parametrs/agregat/addseclevelagregat/index.php"); /* Перенаправление браузера */
