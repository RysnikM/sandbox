...
$sql = "SELECT * FROM agregats WHERE parentid!=0 AND oborid!=0";
$result = $conn->query($sql);
    $r=0;
    while($row = $result->fetch_assoc()) {
        $oborudovanie[$r]=$row;
...
$sql = "SELECT * FROM agregats WHERE id=".$_GET['oborudovanie'].";";
$result = $conn->query($sql);
    $r=0;
    while($row = $result->fetch_assoc()) {
        $oborid=(int)$row['oborid'];
        $r++;
    }
$sql = "INSERT INTO agregats (`name`, `parentid`, `oborid`) VALUES (' ".$_GET['agrparent']."',".$_GET['oborudovanie'].",0);";
$result = $conn->query($sql);
header("Location: /start/parametrs/agregat/addserdlevelagregat/index.php"); /* Перенаправление браузера */
