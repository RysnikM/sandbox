...
chartOEE=Highcharts.chart(idChart, {
    chart: {
    	 height: Hch,
    	   zoomType: 'x',
        type: 'xrange',
    ...
    colors: [clWork, clAlarm, clStop, clPause, planRemont],
    plotOptions: {
    	series: {
    		//pointPadding: 0.1,
    		//pointWidth: 30,
     ...
     $("#rem").css("display","none");
      $("#agr").css("display","none");
                      $("#idgr").val(this.id);
                      $("#obrid").val(sootn[this.options.y].id);
                       $('#prichina').val("");
                    	saveId=this.id;
            ...
            if(this.series.name==="Авария" || this.series.name==="Запланированный ремонт"){
              $("#prich").css("display","block");
  ...
  xAxis: {
    ...
    yAxis: {
        ...
        labels: {
            ...
            useHTML: true,
            formatter: function(){console.log(this); return '<span style="z-index:-5;font:16px Arial, sans-serif;white-space: nowrap;text-transform: uppercase;"><a href = "/start/oee/oee_online/oee_one/index.php?id='+encodeURI(this.value)+'">'+this.value+'</a></span>' },
            style: {
            	// maxWidth: '20px',
              	// whiteSpace: 'nowrap',
