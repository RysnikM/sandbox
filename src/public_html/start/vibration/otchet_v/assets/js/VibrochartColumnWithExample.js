 ...
 var valLevelArr=[[[1500,3000,4566],[1500,3000,4566],[1500,3000,4566]],[[1500,3000,4566],[1500,3000,4566]]];
  ...
  var period=60000;//миллисекунды
   ...
   commonArr[stanokArr[0]]=[commonArr1,commonArr2,commonArr3];
   commonArr[stanokArr[1]]=[commonArr4,commonArr5];
...
for (var i = 0; i < nameArr[h].length; i++) {
     ...
     finishArr[stanokArr[h]][nameArr[h][i]].push({name:nameArr[h][i]+' '+levelArr[0],color:colorArr[0],data:ArrNormalPoint[i],turboThreshold:1000000});    } 
 
      if (ArrWarningPoint[h]!=0){
      finishArr[stanokArr[h]][nameArr[h][i]].push({name:nameArr[h][i]+' '+levelArr[1],color:colorArr[1],data:ArrWarningPoint[i],turboThreshold:1000000});
      } 
      if (ArrStopPoint[h]!=0){
      finishArr[stanokArr[h]][nameArr[h][i]].push({name:nameArr[h][i]+' '+levelArr[2],color:colorArr[2],data:ArrStopPoint[i],turboThreshold:1000000});
      } 
      if (ArrAlarmPoint[h]!=0){
      finishArr[stanokArr[h]][nameArr[h][i]].push({name:nameArr[h][i]+' '+levelArr[3],color:colorArr[3],data:ArrAlarmPoint[i],turboThreshold:1000000});
      } 
...
$("#"+insertId).after('<div class="col-12" id="container'+[i]+[j]+'"></div>');
 ...
 chart[j]=Highcharts.chart('container'+[i]+[j]+'', {
      chart: {      
        type: 'column',
        panning: true,
        animation: false,
